...
int al_ext_init_window(float width_relative, float height_relative)
{
	...
	else
	{
		...
		else
		{
			...
			_al_ext_global.window_width = (monitor_info.x2 - monitor_info.x1) * width_relative;
			_al_ext_global.window_height = (monitor_info.y2 - monitor_info.y1) * height_relative;

			al_set_new_display_flags(ALLEGRO_WINDOWED);
			_al_ext_global.window_handler = al_create_display(_al_ext_global.window_width, 
															  _al_ext_global.window_height); // Crea la ventana
...
int al_ext_init_fullscreen(int width, int height)
{
	...
	else
	{
		...
		else
		{
			...
			_al_ext_global.window_width = width;
			_al_ext_global.window_height = height;
...
int al_ext_init_fullscreen_desktop()
{
	...
	else
	{
		...
		else
		{
			...
			{
				_al_ext_global.timer = al_create_timer(1.0/FPS);
...
static int al_ext_at_exit(void (* func_ptr)(void))
{
	if(_al_ext_global.window_handler)
	{
   		al_destroy_display(_al_ext_global.window_handler);
   		al_destroy_event_queue(_al_ext_global.event_queue);
	}
